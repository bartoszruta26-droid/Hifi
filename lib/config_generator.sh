...
[[ -n "${_CONFIG_GEN_SH_LOADED:-}" ]] && return 0
readonly _CONFIG_GEN_SH_LOADED=1
...
# ==========================================
# MAPOWANIE BIT_DEPTH → OUTPUT_FORMAT
# ==========================================
# float64le zapewnia najwyższą precyzję wewnętrzną PulseAudio niezależnie
# od głębi bitowej DAC. Użyj s32le/s24le/s16le jeśli zależy Ci na oszczędności CPU.
resolve_output_format() {
    ...
    case "$bit_depth" in
        ...
        24) echo "s24le" ;;
        32) echo "s32le" ;;
        *)  echo "float64le" ;;   # domyślnie: maksymalna precyzja przetwarzania
    esac
}
...
generate_daemon_conf() {
    ...
    # Wyznacz format wyjściowy na podstawie wybranej głębi bitowej
    OUTPUT_FORMAT="$(resolve_output_format "$BIT_DEPTH")"
...
#
# Uwaga: format wyjściowy odpowiada wybranej głębi bitowej (${BIT_DEPTH} bit → ${OUTPUT_FORMAT}).
# Aby uzyskać maksymalną precyzję wewnętrzną kosztem wyższego zużycia CPU,
# możesz ręcznie zmienić default-sample-format na float64le.
...
EOF
    ...
    # Zwracamy tylko ścieżkę — log() pisze na stderr, nie zatruwa $()
    echo "$output_file"
}
...
# Detekcja sprzętu (tsched=0 dla lepszej synchronizacji z DAC HAT)
load-module module-udev-detect tsched=0

# ALSA jako backend
load-module module-alsa-sink

# Always sink dla stabilności (zapobiega zawieszeniu gdy brak aktywnego sinka)
load-module module-always-sink

# Intended roles dla lepszego routingu audio
...
# Uwaga: NIE ustawiamy set-default-sink — po wyłączeniu onboard audio
# (dtparam=audio=off) wbudowany sink nie istnieje. module-udev-detect
# automatycznie wykryje DAC HAT jako domyślny sink.
EOF
...
generate_mpd_conf() {
    ...
    if [[ -f "$MPD_CONF" ]]; then
        ...
        sed -i '/^auto_update_depth/d'     "$output_file"
...
generate_config_txt() {
    ...
    if [[ -n "$source_cfg" ]] && [[ -f "$source_cfg" ]]; then
        ...
        # POPRAWKA: sed -E (ERE) — czytelniejszy i pewniejszy niż BRE
        sed -E -i '/^dtoverlay[[:space:]]*=[[:space:]]*(hifiberry|justboom|iqaudio|allo-boss|allo-katana|googlevoicehat|audioinjector|i2s-dac)/d' "$output_file"
        sed -E -i '/^dtparam[[:space:]]*=[[:space:]]*audio=/d'      "$output_file"
        sed -E -i '/^dtparam[[:space:]]*=[[:space:]]*i2c/d'         "$output_file"
        sed -E -i '/^dtparam[[:space:]]*=[[:space:]]*spi=/d'        "$output_file"

        {
            ...
            # POPRAWKA: I2C tylko dla DAC-ów które go wymagają
            if hat_requires_i2c "$HAT_MODEL"; then
        ...
        {
            echo "# === RPi4 Audio HAT Configuration ==="
...
gen_configs() {
    ...
    if ! validate_hat_model "$HAT_MODEL"; then
        echo -e "${COLORS[YELLOW]}⚠️  Ostrzeżenie: Model '$HAT_MODEL' może być niepoprawny${COLORS[NC]}"
    fi

    # Generuj wszystkie pliki — log() pisze na stderr, $() przechwytuje tylko ścieżki
    ...
    echo "  • Resample      : ${RESAMPLE_METHOD}"
    echo "  • DAC Overlay   : ${HAT_MODEL}"
    if hat_requires_i2c "$HAT_MODEL"; then
        echo "  • I2C           : włączone (wymagane przez ${HAT_MODEL})"
    else
        echo "  • I2C           : wyłączone (niewymagane przez ${HAT_MODEL})"
    fi
    echo ""
...
preview_configs() {
    ...
    for file in "$STAGING_DIR"/*.new; do
        if [[ -f "$file" ]]; then
...
export -f resolve_output_format generate_daemon_conf generate_default_pa \
           generate_mpd_conf generate_config_txt gen_configs preview_configs
